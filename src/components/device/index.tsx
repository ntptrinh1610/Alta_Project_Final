...
const DeviceList = React.memo((props: DeviceListProps) => {
  ...
  return (
    <div className="device-list">
            ...
            <Input
              ...
              onChange={async (e) => {
                if (props.columns == 4 || props.columns == 3)
                  ...
                  setDisplayData(filter1Value != 'All' ? filter2Value != 'All' ? internalData.filter((x: any) => x.deviceName.toLowerCase().includes((e.target.value).toLowerCase()) && x.operationStatus == filter1Value && x.connected == filter2Value)
                    : internalData.filter((x: any) => x.deviceName.toLowerCase().includes((e.target.value).toLowerCase()) && x.operationStatus == filter1Value)
                    : internalData.filter((x: any) => x.deviceName.toLowerCase().includes((e.target.value).toLowerCase()))
                  )
                }
                else if (props.columns == 2) {
                  setDisplayData(filter1Value != "All" ? internalData.filter((x: any) => x.description.toLowerCase().includes((e.target.value).toLowerCase()) && x.isInOperation == filter1Value)
                    : internalData.filter((x: any) => x.description.toLowerCase().includes((e.target.value).toLowerCase())))
                }
      ...
      <Modal title="Confirm Delete" open={isModelDeleteOpen} onOk={handleDelete} onCancel={() => { setIsModelDeleteOpen(false) }}
