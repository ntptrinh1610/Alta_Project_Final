...
const AccountForm = (props: AccountFormProps) => {
  ...
  const initialValues = Object.keys(props.myForm).length === 0 ?
    ...
    : {
      ...
      userRole: props.myForm.userRole,
      confirmPassword: '',
    }
  useEffect(() => {
    ...
    getDataSvc();
    if (props.myForm.userRole == 'Doctor') {
        ...
        <Row gutter={24}>
          ...
          <Col xs={24} lg={12}>
            <Form.Item
              name="email"
              label="Địa chỉ mail"
              rules={[{ required: true, message: 'Email là bắt buộc' }, { validator: validateEmail }]}
            >
              <Input placeholder="Nhập email" disabled={isDisableInput}
              />
            </Form.Item>
            <Form.Item
              name="fullName"
            ...
            <Form.Item
              name="phoneNumber"
              label="Số điện thoại"
              rules={[{ required: true, message: 'Số điện thoại là bắt buộc' }, { validator: validatePhoneNumber }]}
            >
